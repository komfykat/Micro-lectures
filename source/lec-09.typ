...
#let IE = math.op("IE")
#underline[
  = Свойства функции расходов (продолжение).
]
...
$
  "UMP":
  cases(u(x) -> display(max_(x >= 0)),
        p x <= m) => x(p, m) - "маршаллинский спрос", cal(V)(p, m) - "косвенная функция полезности"
$
$
  "EMP":
  cases(p x -> display(min_(x >= 0)),
        u(x) >= u) => x^h (p, u) - "хиксианский спрос", e(p, u) - "функция расходов"
$
#statement[
